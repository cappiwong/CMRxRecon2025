...
#             # Combine coils with Root Sum of Squares (RSS)
#             image_rss = np.sqrt(np.sum(np.abs(image) ** 2, axis=2))  # shape: [frames, slices, height, width]
...
#             # === Fix orientation ===
#             output_img = np.flipud(np.rot90(output_img))  # rotate 90° and flip vertically
...
# === Output directories ===
output_png_dir = "output_imageflip_sax"
output_npy_dir = "output_numpyflip_sax"
os.makedirs(output_png_dir, exist_ok=True)
os.makedirs(output_npy_dir, exist_ok=True)
...
for i in range(1, 62):
    patient_id = f"P{i:03}"  # P001 to P061
    mat_file = os.path.join(fullsample_base, patient_id, "cine_sax.mat")
    ...
    try:
        with h5py.File(mat_file, 'r') as hf:
            ...
            # === Fix orientation: rotate 90° counterclockwise, then flip vertically ===
            image_rss_corrected = np.flip(np.rot90(image_rss, axes=(-2, -1)), axis=-2)

            # === Save corrected numpy array ===
            npy_path = os.path.join(output_npy_dir, f"{patient_id}_cine_sax.npy")
            np.save(npy_path, image_rss_corrected)

            # === Save one sample PNG (frame 0, slice 0) ===
            frame_idx, slice_idx = 0, 0
            output_img = np.abs(image_rss_corrected[frame_idx, slice_idx])
            plt.imshow(output_img, cmap='gray')
            plt.title(f"{patient_id} f{frame_idx} s{slice_idx}")
            plt.axis('off')
